#!/usr/bin/env python3
"""
GPU-accelerated N-body gravitational force computation using CuPy.

This module provides high-performance direct N-body force calculation on NVIDIA GPUs
...
void nbody_forces_kahan_kernel(
    ...
    const {T}* __restrict__ mass, const {T}* __restrict__ h,
    {T}* __restrict__ ax, {T}* __restrict__ ay, {T}* __restrict__ az,
    {T} G, {T} eps2, int kernel_id, int N
) {{
    __shared__ {T} sh_x[TILE_SIZE];
    __shared__ {T} sh_y[TILE_SIZE];
    __shared__ {T} sh_z[TILE_SIZE];
    ...
    {T} my_x = x[i]; {T} my_y = y[i]; {T} my_z = z[i]; {T} my_h = h[i];
    
    // Sum and compensation variables
    {T} sum_x = 0.0, c_x = 0.0;
    {T} sum_y = 0.0, c_y = 0.0;
    {T} sum_z = 0.0, c_z = 0.0;
    ...
    for (int tile = 0; tile < num_tiles; tile++) {{
        ...
        if (j < N) {{
            sh_x[threadIdx.x] = x[j]; sh_y[threadIdx.x] = y[j];
            sh_z[threadIdx.x] = z[j]; sh_m[threadIdx.x] = mass[j];
            sh_h[threadIdx.x] = h[j];
        }} else {{ sh_m[threadIdx.x] = 0.0; }}
        __syncthreads();

        for (int k = 0; k < TILE_SIZE && (tile * TILE_SIZE + k) < N; k++) {{
            if (i == (tile * TILE_SIZE + k)) continue;
            
            {T} dx = sh_x[k] - my_x;
            {T} dy = sh_y[k] - my_y;
            {T} dz = sh_z[k] - my_z;
            {T} r2 = dx*dx + dy*dy + dz*dz + eps2;
            {T} kern = compute_kernel_factor(r2, max(my_h, sh_h[k]), kernel_id);
            {T} mj = sh_m[k];
            
            // Kahan logic for X
            {T} val_x = (mj * kern) * dx;
            {T} y_comp_x = val_x - c_x;
            {T} t_x = sum_x + y_comp_x;
            c_x = (t_x - sum_x) - y_comp_x;
            sum_x = t_x;

            // Kahan logic for Y
            {T} val_y = (mj * kern) * dy;
            {T} y_comp_y = val_y - c_y;
            {T} t_y = sum_y + y_comp_y;
            c_y = (t_y - sum_y) - y_comp_y;
            sum_y = t_y;

            // Kahan logic for Z
            {T} val_z = (mj * kern) * dz;
            {T} y_comp_z = val_z - c_z;
            {T} t_z = sum_z + y_comp_z;
            c_z = (t_z - sum_z) - y_comp_z;
            sum_z = t_z;
        }}
        __syncthreads();
    }}
    ax[i] = G * sum_x; ay[i] = G * sum_y; az[i] = G * sum_z;
}}
'''


# Compile kernel on first import (if CuPy available)
# At module level, after kernel definition
# if CUPY_AVAILABLE:
#     _nbody_kernel = cp.RawKernel(_NBODY_CUDA_SOURCE, 'nbody_forces_kernel')
    
#     # Auto-compile on import (silent, tiny warmup)
#     try:
#         _dummy_pos = cp.ones((10, 3), dtype=cp.float32)
#         _dummy_mass = cp.ones(10, dtype=cp.float32)
#         _dummy_h = cp.ones(10, dtype=cp.float32)
#         _dummy_acc_x = cp.empty(10, dtype=cp.float32)
#         _dummy_acc_y = cp.empty(10, dtype=cp.float32)
#         _dummy_acc_z = cp.empty(10, dtype=cp.float32)
        
#         # Trigger compilation (1 thread, instant)
#         _nbody_kernel((1,), (1,), (
#             _dummy_pos[:, 0], _dummy_pos[:, 1], _dummy_pos[:, 2],
#             _dummy_mass, _dummy_h,
#             _dummy_acc_x, _dummy_acc_y, _dummy_acc_z,
#             cp.float32(1.0), cp.float32(1e-8), cp.int32(0), cp.int32(10)
#         ))
#         cp.cuda.Stream.null.synchronize()
        
#         # Clean up dummy arrays
#         del _dummy_pos, _dummy_mass, _dummy_h, _dummy_acc_x, _dummy_acc_y, _dummy_acc_z
#     except Exception:
#         pass  # If it fails, kernel will compile on first real use
# else:
#     _nbody_kernel = None

def _get_nbody_kernel(precision='float64'):
    # 1. Check Cache
    if precision in _KERNEL_CACHE:
        return _KERNEL_CACHE[precision]
    
    # 2. Define Type Specs (Math functions for float vs double)
    specs = {
        'float32': {
            'T': 'float', 'RSQRT': 'rsqrtf', 'SQRT': 'sqrtf', 
            'FMA': 'fmaf', 'FMAX': 'fmaxf'
        },
        'float64': {
            'T': 'double', 'RSQRT': 'rsqrt', 'SQRT': 'sqrt', 
            'FMA': 'fma', 'FMAX': 'fmax'
        },
    }

    # 3. Select Template and Kernel Name based on input
    if precision == 'float32_kahan':
        # Use Kahan Logic
        template = _KAHAN_KERNEL_TEMPLATE
        kernel_name = 'nbody_forces_kahan_kernel' # Must match C++ void name
        type_key = 'float32' # Kahan uses float32 math types
        
    else:
        # Use Standard Logic (float32 or float64)
        template = _NBODY_KERNEL_TEMPLATE
        kernel_name = 'nbody_forces_kernel'         # Must match C++ void name
        type_key = precision                        # 'float32' or 'float64'

    # 4. Inject types into the selected template
    # We look up specs using 'type_key' (stripping away _kahan)
    s = specs[type_key]
    source = template.format(**s)
    
    # 5. Compile
    # Important: The second argument must match the function name in C++!
    kernel = cp.RawKernel(source, kernel_name)
    
    # 6. Cache and Return
    _KERNEL_CACHE[precision] = kernel
    return kernel
...
def compute_nbody_forces_gpu(
    ...
    G: float = G_DEFAULT,
    precision: Literal['float32', 'float64', 'float32_kahan'] = 'float32',
    ...
    _nbody_kernel = _get_nbody_kernel(precision)

    # Launch kernel
    threads_per_block = 256
    blocks_per_grid = (N + threads_per_block - 1) // threads_per_block
    
    eps2 = 1e-8  # Regularization parameter
    ...
    if return_cupy:
        ...
        acc = np.empty((N, 3), dtype=dtype_np)
        acc[:, 0] = cp.asnumpy(ax_gpu)
        acc[:, 1] = cp.asnumpy(ay_gpu)
        acc[:, 2] = cp.asnumpy(az_gpu)
        return acc
