# Graph Report - Nbody_streams  (2026-07-29)

## Corpus Check
- 91 files · ~171,685 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2228 nodes · 3995 edges · 270 communities (157 shown, 113 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 326 edges (avg confidence: 0.59)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fd3ddc2c`
...
Cohesion: 0.05
Nodes (31): DiskAnsatzPotentialGPU, LogHaloPotentialGPU, DiskAnsatz separable disk potential: Phi(R,z) = f(r) * H(z)     where r = sqrt(R, LogHalo: Phi = (v0^2/2) * ln(rc^2 + x^2 + y^2/p^2 + z^2/q^2)      Constructor ma, MultipolePotentialGPU, GPU evaluator for an Agama Multipole BFE potential.      Replicates Agama's ``Mu, Scalar args common to all kernels.          All int/double scalars are explicitl, _pts() (+23 more)
...
Cohesion: 0.11
Nodes (27): _detect_expansion_type(), Path, agama_helper._coefs ~~~~~~~~~~~~~~~~~~~ Structured representations of Agama expa, Return lines from any coef source: file path, HDF5 path, or raw string.      Del, Return 'Multipole' or 'CylSpline' from a coef string header, or '' if unknown., Read an Agama Multipole expansion into a :class:`MultipoleCoefs` dataclass., Read an Agama CylSpline expansion into a :class:`CylSplineCoefs` dataclass., Read an Agama expansion coefficient file into a structured dataclass.      The e (+19 more)
...
Cohesion: 0.09
Nodes (24): _clamped_left_cubic_deriv_batch(), CylSplinePotentialGPU, _get_cylspl_kernel(), _get_kernel(), _natural_cubic_deriv_batch(), _prep_xyz(), ndarray, First derivatives at all knots for natural cubic splines — batched over columns. (+16 more)

### Community 11 - "ShiftedPotentialGPU"
Cohesion: 0.09
Nodes (11): _apply_modifiers(), Agama-compatible eval : see ``MultipolePotentialGPU.eval`` for details., Agama-compatible eval : see ``MultipolePotentialGPU.eval`` for details., Shifted modifier: evaluates ``inner`` at ``xyz - center(t)``.      Parameters, Scaled modifier: ``Phi_s(x,t) = a(t)*s(t) * Phi(x*s(t))`` where ``s(t) = 1/scale, Return (s, a) = (1/scale(t), ampl(t)) with linear extrapolation., Wrap *pot* with Shifted/Scaled modifiers if requested.      ``scale`` may be:, Agama-compatible eval : returns any combination of potential, acceleration, (+3 more)
...
Cohesion: 0.09
Nodes (15): _AnalyticBase, AnalyticPotentialGPU(), PlummerPotentialGPU, _prep_xyz(), ndarray, _analytic_potentials.py ~~~~~~~~~~~~~~~~~~~~~~~ Phase 2: GPU-accelerated analyti, Uniform (spatially constant) acceleration: F = (ax, ay, az).     Phi(x,y,z) = -a, Factory function matching Agama's constructor syntax::          pot = AnalyticPo (+7 more)

### Community 15 - "tree_gravity_gpu"
Cohesion: 0.14
Nodes (20): generate_plummer(), Net force = sum(m_i * a_i) should be ~0 for an isolated system., Net torque = sum(r x (m*a)) should be small., Compare 0.5*sum(m*phi_tree) against 0.5*sum(m*phi_direct)., For a spherically symmetric distribution, forces should point inward., Error should not degrade significantly as N grows (same distribution)., Plummer model ICs matching the C++ binary (double precision + R<100 rejection)., Calling tree_gravity_gpu twice with identical input should give identical output (+12 more)
...
Cohesion: 0.05
Nodes (38): DehnenSphericalPotentialGPU, MiyamotoNagaiPotentialGPU, Dehnen spherical: Phi(r) = -(GM/a) * (1 - (r/(r+a))^(2-gamma)) / (2-gamma)     S, Miyamoto-Nagai disk: Phi = -GM / sqrt(R^2 + (sqrt(z^2+b^2)+a)^2)     where R^2 =, _build_dehnen_gpu(), _build_disk_gpu(), _build_king_gpu(), _build_multipole_data() (+30 more)
...
Cohesion: 0.10
Nodes (23): ndarray, nbody_streams.sim Unified high-level simulation entry point for multi-species N-, Run a direct N-body simulation with one or more particle species.      This is t, run_simulation(), _build_particle_arrays(), _emit_performance_warnings(), PerformanceWarning, NDArray (+15 more)

### Community 43 - "Box"
Cohesion: 0.10
Nodes (18): double3, Box, centre, __device__, hsize, float3, __host__, T (+10 more)

### Community 44 - "_force.py"
Cohesion: 0.13
Nodes (21): _coerce_eps(), _gpu_ptr(), _max_n(), ndarray, _force.py  --  ctypes interface to the GPU tree-code gravity shared library.  Su, Return a contiguous float32 CuPy array of length n for eps.      Accepts:, Compute gravitational accelerations and potential using the GPU tree code     (B, _ResInteractions (+13 more)
...
Cohesion: 0.10
Nodes (17): NFWPotentialGPU, Navarro-Frenk-White potential optimized with CuPy ElementwiseKernels., CompositePotentialGPU, PotentialGPU(), Sum of GPU potential components : mirrors agama.Potential composite.      Each c, GPU potential factory : mirrors the ``agama.Potential`` API.      Usage     ----, Write a potential config file with the given suffix (any extension works)., _is_potential_ini detects multi-section content regardless of extension. (+9 more)

### Community 47 - "_random_xv"
Cohesion: 0.17
Nodes (9): Write a snapshot compatible with :class:`ParticleReader`.      Two modes     ---, _save_snapshot(), ndarray, _random_xv(), Write an HDF5 file in the old two-species format., Old file with only dark particles (no star group)., Random (N, 6) phase space  - tiny, just for IO tests., TestParticleReaderBackwardCompat (+1 more)
...
Cohesion: 0.18
Nodes (15): _add_negative_m(), generate_lmax_pairs(), Generate (l, m) pairs for a spherical harmonic expansion.      Parameters     --, Expand (l, m) pairs to include their negative-m counterparts.      Each (l, m) w, create_fire_evolving_ini(), load_fire_pot(), Path, agama_helper._fire ~~~~~~~~~~~~~~~~~~ FIRE-simulation-specific convenience wrapp (+7 more)
...
Cohesion: 0.09
Nodes (14): HernquistPotentialGPU, IsochronePotentialGPU, Hernquist: Phi(r) = -GM / (r + a)      Constructor: HernquistPotentialGPU(mass=1, Isochrone: Phi(r) = -GM / (b + sqrt(r^2 + b^2))      Constructor: IsochronePoten, _frel(), GPUTimer, _hess_fd_check(), _pts() (+6 more)
...
Cohesion: 0.16
Nodes (13): _build_single(), _is_potential_ini(), _load_potential_ini(), _pop_ci(), Path, Load from an Agama .coef_cylsp file., Build from an agama.Potential by materialising it as a single Multipole, Case-insensitive pop from *d* (first matching key wins). (+5 more)
...
Cohesion: 0.15
Nodes (9): cuda_alive(), Return True if the CUDA context has no pending errors.      Uses cudaGetLastErro, nbody_streams.tree_gpu ====================== GPU Barnes-Hut tree code for N-bod, ndarray, nbody_streams.tree_gpu.run_gpu_tree ==================================== KDK lea, Run a GPU-tree N-body simulation with KDK leapfrog integration.      Force evalu, Background-thread watchdog for GPU leapfrog loops.      Per-step overhead is ~1, run_nbody_gpu_tree() (+1 more)
...
Cohesion: 0.40
Nodes (5): compute_nbody_forces_gpu, Example, Parameters, Raises, Returns

### Community 64 - "vec<2,double>"
Cohesion: 0.50
Nodes (3): double2, vec<2,double>, __device__
...
Cohesion: 0.67
Nodes (3): CDLL, _find_cudart(), Load libcudart; try versioned names as fallback.
...
Cohesion: 0.16
Nodes (8): CylSplineCoefs, Structured representation of a CylSpline (azimuthal harmonic BFE) potential., Return a copy with all m terms **not** in *keep_m* zeroed out.          Paramete, Serialise back to the Agama CylSpline text format.          Returns         ----, EvolvingPotentialGPU, _lerp(), Time-evolving GPU potential: wraps a sequence of static GPU potentials     at kn, Linear interp a*(1-alpha) + b*alpha.
...
Cohesion: 0.31
Nodes (12): load_agama_potential(), Load a single-snapshot Agama potential from any coefficient source.      Accepts, _pts(), test_phase3_cylspline.py ~~~~~~~~~~~~~~~~~~~~~~~~ Tests CylSplinePotentialGPU ag, Random Cartesian points inside a sphere, avoiding z-axis., _rel_deriv(), _rel_force(), _rel_phi() (+4 more)
...
Cohesion: 0.10
Nodes (20): compute_nbody_forces_cpu, compute_nbody_potential_cpu, compute_nbody_potential_gpu, Contents, Example, Example, Example, get_gpu_info (+12 more)
...
- **Why does `_GPUPotBase` connect `_AnalyticBase` to `MultipolePotentialGPU`, `make_df_force_extra`, `_potential.py`, `EvolvingPotentialGPU`, `_jeans_sigma_r`, `CylSplinePotentialGPU`, `ShiftedPotentialGPU`, `NFWPotentialGPU`, `test_phase2_analytic.py`, `CompositePotentialGPU`, `.radial_power`, `Generate num_pts points on a sphere using the Fibonacci spiral.      Points are`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `Species` connect `Species` to `_make_ic`, `.dark`, `test_multi_species.py`, `_random_xv`, `test_phase2_analytic.py`, `Fit an adaptive ellipsoid to a particle distribution and compute shape diagnosti`, `Generate uniformly random points on the surface of a sphere.      Parameters`, `Generate num_pts points on a sphere using the Fibonacci spiral.      Points are`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `PotentialGPU()` connect `NFWPotentialGPU` to `MultipolePotentialGPU`, `_load.py`, `_potential.py`, `test_phase3_cylspline.py`, `ShiftedPotentialGPU`, `r"""Fit a spheroid (Zhao / generalised double-power-law) density profile.      I`, `CompositePotentialGPU`, `Ellipsoidal radius and particle selection (numba-accelerated).      Computes the`?**
