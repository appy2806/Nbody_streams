...
## Corpus Check
- 96 files · ~191,084 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2534 nodes · 4584 edges · 287 communities (173 shown, 114 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 337 edges (avg confidence: 0.59)
...
5. `PotentialGPU()` - 44 edges
6. `CylSplinePotentialGPU` - 41 edges
7. `MultipoleCoefs` - 40 edges
8. `CompositePotentialGPU` - 40 edges
9. `CylSplineCoefs` - 38 edges
10. `tree_gravity_gpu()` - 38 edges
...
## Communities (287 total, 114 thin omitted)
...
Cohesion: 0.07
Nodes (13): DiskAnsatzPotentialGPU, PlummerPotentialGPU, DiskAnsatz separable disk potential: Phi(R,z) = f(r) * H(z)     where r = sqrt(R, Plummer sphere: Phi(r) = -GM / sqrt(r^2 + b^2)      Constructor: PlummerPotentia, CompositePotentialGPU, _GPUPotBase, Sum of GPU potential components : mirrors agama.Potential composite.      Each c, Build from an agama.Potential by materialising it as a single Multipole (+5 more)
...
Cohesion: 0.13
Nodes (13): _build_particle_arrays(), NDArray, nbody_streams.species Species definitions and helper functions for multi-species, Split a combined ``(N_total, 6)`` phase-space array into per-species slices., Definition of a particle species in an N-body simulation.      Parameters     --, Build combined per-particle mass and softening arrays from a species list., Species, _split_by_species() (+5 more)
...
Cohesion: 0.12
Nodes (17): CylSplinePotentialGPU, _get_cylspl_kernel(), _get_kernel(), _prep_xyz(), ndarray, GPU evaluator for an Agama CylSpline potential.      Replicates Agama's CylSplin, Evaluate rho via Laplacian: rho = -(Hxx+Hyy+Hzz)/(4piG)., Accept (3,) or (N,3) numpy/cupy array. Returns (cp float64 (N,3), was_single). (+9 more)
...
Cohesion: 0.12
Nodes (16): _AnalyticBase, AnalyticPotentialGPU(), _prep_xyz(), ndarray, _analytic_potentials.py ~~~~~~~~~~~~~~~~~~~~~~~ Phase 2: GPU-accelerated analyti, Hyman (1983) slope limiter applied to natural-cubic-spline derivatives.      Por, Spatially uniform, optionally time-dependent acceleration ``a(t)``, arising, Acceleration components at time *t*. (+8 more)
...
Cohesion: 0.10
Nodes (33): nbody_streams - lightweight direct N-body utilities., _load_restart(), Path, nbody_streams.nbody_io  I/O utilities for N-body snapshots and restart data.  Pr, Append or update snapshot.times in output_dir.     Ensures unique snap_index ent, Save a restart file for crash recovery.      Parameters     ----------     phase, Load a restart file if one exists.      Returns     -------     tuple of length, _save_restart() (+25 more)
...
Cohesion: 0.03
Nodes (44): Stack time-less coefficient snapshots into one object with a time axis.      Gri, Read an Agama expansion into a structured dataclass — one snapshot or many., read_coefs(), stack_coefs(), cylsp3_series(), cylsp3_snaps(), cylsp_series(), cylsp_snaps() (+36 more)
...
Cohesion: 0.05
Nodes (39): HernquistPotentialGPU, Hernquist: Phi(r) = -GM / (r + a)      Constructor: HernquistPotentialGPU(mass=1, _build_cylspline_data(), _build_dehnen_gpu(), _build_disk_gpu(), _build_king_gpu(), _build_multipole_data(), _build_spheroid_gpu() (+31 more)
...
Cohesion: 0.24
Nodes (8): _emit_performance_warnings(), PerformanceWarning, Emit :class:`PerformanceWarning` when *N_total* exceeds recommended     threshol, Warning emitted when particle count exceeds recommended thresholds., Tree method on CPU never warns for moderate N., run_simulation emits PerformanceWarning for large CPU direct N., TestPerformanceWarnings, UserWarning

### Community 43 - "Box"
Cohesion: 0.09
Nodes (17): MultipoleCoefs, Read one or many Agama Multipole snapshots into a :class:`MultipoleCoefs`., Structured representation of a Multipole (spherical harmonic BFE) potential., Maximum l order present in *lm_labels*., Sorted unique l values., Sorted unique m values (includes negatives)., Return a copy with all (l, m) terms **not** in *keep_lm* zeroed out.          Ne, Return a deep copy — no array, list or dict is shared with *self*. (+9 more)

### Community 44 - "_force.py"
Cohesion: 0.12
Nodes (17): _coerce_eps(), _gpu_ptr(), ndarray, Return a contiguous float32 CuPy array of length n for eps.      Accepts:, _ResInteractions, generate_plummer(), Time the tree_gravity_gpu() Python helper function., Plummer model ICs matching the C++ binary (double precision + R<100 rejection). (+9 more)

### Community 45 - "buildTree.cu"
Cohesion: 0.06
Nodes (33): _as_existing_path(), _check_unique_names(), _CoefTimeAxisMixin, _is_source_sequence(), Path, Return lines from any coef source: file path, HDF5 path, or raw string.      Del, Time-less view at time index *i*.          The returned object shares memory wit, Assert internal shape consistency, naming the offending field on failure. (+25 more)

### Community 46 - "NFWPotentialGPU"
Cohesion: 0.06
Nodes (23): NFWPotentialGPU, Navarro-Frenk-White potential optimized with CuPy ElementwiseKernels., _pts(), gpu=True from a CylSpline coef file → CylSplinePotentialGPU., gpu=True from a MultipoleCoefs dataclass → MultipolePotentialGPU., gpu=True + keep_lm_mult: GPU and CPU should agree after filtering., gpu=True with center= should return ShiftedPotentialGPU., Default gpu=False returns agama.Potential, not a GPU object. (+15 more)

### Community 47 - "_random_xv"
Cohesion: 0.14
Nodes (11): Write a snapshot compatible with :class:`ParticleReader`.      Two modes     ---, _save_snapshot(), Validate *species* list against *phase_space* shape.      Checks     ------, _validate_species(), ndarray, _random_xv(), Old file with only dark particles (no star group)., Random (N, 6) phase space  - tiny, just for IO tests. (+3 more)
...
Cohesion: 0.09
Nodes (14): DehnenSphericalPotentialGPU, IsochronePotentialGPU, Dehnen spherical: Phi(r) = -(GM/a) * (1 - (r/(r+a))^(2-gamma)) / (2-gamma)     S, Isochrone: Phi(r) = -GM / (b + sqrt(r^2 + b^2))      Constructor: IsochronePoten, _frel(), GPUTimer, _hess_fd_check(), _pts() (+6 more)

### Community 55 - "Fit an adaptive ellipsoid to a particle distribution and compute shape diagnosti"
Cohesion: 0.19
Nodes (9): _is_uniform(), ndarray, Worker for N-species parallel extraction.      Args (tuple):         snap_index, Check whether all elements of *arr* are equal to within relative tolerance., Worker executed in a separate process. Reads one snapshot from disk and writes, Extract orbits for selected particle types across snapshots,         using paral, _worker_write_shared(), _worker_write_shared_multi() (+1 more)
...
Cohesion: 0.10
Nodes (15): CDLL, cuda_alive(), _find_cudart(), _max_n(), _force.py  --  ctypes interface to the GPU tree-code gravity shared library.  Su, Pre-allocated GPU tree handle for time-stepping loops.      Parameters     -----, Load libcudart; try versioned names as fallback., Return True if the CUDA context has no pending errors.      Uses cudaGetLastErro (+7 more)
...
Cohesion: 0.13
Nodes (5): MiyamotoNagaiPotentialGPU, Miyamoto-Nagai disk: Phi = -GM / sqrt(R^2 + (sqrt(z^2+b^2)+a)^2)     where R^2 =, King is exported as a spherical (lmax=0) Multipole BFE by Agama.     Agreement v, _rel_force(), TestKingGPUAccuracy
...
Cohesion: 0.08
Nodes (36): CylSplineCoefs, Return a copy with all m terms **not** in *keep_m* zeroed out.          Paramete, Return a deep copy — no array, list or dict is shared with *self*., Split a CylSpline line stream into ``#Phi`` / ``#dPhi/dR`` / ``#dPhi/dz`` blocks, Parse **one** Agama CylSpline snapshot into a :class:`CylSplineCoefs`.      Inte, Dispatch one coef string to the parser named by its ``type=`` header., Read one or many Agama CylSpline snapshots into a :class:`CylSplineCoefs`., Structured representation of a CylSpline (azimuthal harmonic BFE) potential. (+28 more)
...
Cohesion: 0.12
Nodes (18): _build_single(), _coerce(), _is_potential_ini(), _load_potential_ini(), MultipolePotentialGPU, _pop_ci(), Path, Load from an Agama .coef_cylsp file. (+10 more)
...
Cohesion: 0.15
Nodes (6): LogHaloPotentialGPU, LogHalo: Phi = (v0^2/2) * ln(rc^2 + x^2 + y^2/p^2 + z^2/q^2)      Constructor ma, PotentialGPU factory should accept type= and kwargs in any case., type='nfw', 'NFW', 'Nfw' should all work., Logarithmic potential: v0= (Agama alias) maps to velocity= (GPU param)., TestPotentialGPUCaseInsensitive
...
### Community 89 - "_io.py"
Cohesion: 0.07
Nodes (54): _add_negative_m(), _detect_expansion_type(), generate_lmax_pairs(), agama_helper._coefs ~~~~~~~~~~~~~~~~~~~ Structured representations of Agama expa, Return 'Multipole' or 'CylSpline' from a coef string header, or '' if unknown., Generate (l, m) pairs for a spherical harmonic expansion.      Parameters     --, Expand (l, m) pairs to include their negative-m counterparts.      Each (l, m) w, create_fire_evolving_ini() (+46 more)
...
Cohesion: 0.07
Nodes (50): _AgamaTimeSpline, Time interpolation of a K-vector, bit-compatible with Agama.      Reproduces ``p, Value of the K-vector at scalar time *t*, as a tuple of K floats., Build the acceleration spline from Agama's ``file=`` argument.      Accepts what, _read_accel_table(), _normalize_params(), PotentialGPU(), Return *d* with keys mapped to their canonical camelCase equivalents.      Keys (+42 more)
...
- **Why does `_GPUPotBase` connect `MultipolePotentialGPU` to `vec<2,float>`, `make_df_force_extra`, `_load.py`, `_potential.py`, `EvolvingPotentialGPU`, `TreeGPU`, `CylSplinePotentialGPU`, `Box`, `ShiftedPotentialGPU`, `_AnalyticBase`, `NFWPotentialGPU`, `test_phase2_analytic.py`, `Ellipsoidal radius and particle selection (numba-accelerated).      Computes the`, `Find the density peak by locating the minimum gravitational potential.      Uses`, `TreeGPU`, `test_chandrasekhar.py`?**
...
- **Why does `CylSplineCoefs` connect `TreeGPU` to `vec<2,float>`, `MultipolePotentialGPU`, `_potential.py`, `EvolvingPotentialGPU`, `CylSplinePotentialGPU`, `ShiftedPotentialGPU`, `buildTree.cu`, `test_newtons_third_law.py`, `.radial_power`, `_io.py`, `vec<3,double>`?**
